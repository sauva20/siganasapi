...
export const saveGradingResult = async (req: AuthRequest, res: Response) => {
  try {
    const { batch_id, yolo_result } = req.body;
    const batchIdNum = Number(batch_id);
    ...
    const dssInput = {
      deteksi_ukuran: yolo_result.sizeScore || "Besar",
      deteksi_warna_kulit: yolo_result.colorScore || "Kuning",
      deteksi_kematangan_pct: yolo_result.kematangan || 80,
      kondisi_mahkota: yolo_result.shapeScore || "Sempurna",
      kondisi_defect: yolo_result.defectScore || "Tidak Ada Cacat"
    };

    const dssOutput = {
      grade_mutu: yolo_result.grade === 'A' ? 'grade_a' : yolo_result.grade === 'B' ? 'grade_b' : yolo_result.grade === 'C' ? 'grade_c' : 'reject',
      rekomendasi_pasar: yolo_result.grade === 'A' ? 'Ekspor' : 'Lokal',
      estimasi_harga_min: 5000,
      estimasi_harga_max: 15000
    };
    ...
    const incC = dssOutput.grade_mutu === "grade_c" ? 1 : 0;
    const incR = dssOutput.grade_mutu === "reject" ? 1 : 0;
    const addedBerat = yolo_result.weight || 1.0;
    ...
    res.status(201).json({ grading, dss_output: dssOutput, yolo_result });
  } catch (error: any) {
    console.error(error.message || error);
    res.status(500).json({ detail: "Internal server error" });
  }
